<template>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b px-6 py-4">
        <div>
            ...
            <p class="text-sm text-muted-foreground">Manage and monitor your upcoming events</p>
        </div>
    ...
    <div class="p-6">
        ...
        <div v-if="!events.length" class="flex flex-col items-center justify-center py-24 text-center px-4">
            ...
            <p class="text-sm text-slate-500 mt-2 mb-6 max-w-sm">You haven't created any events yet. Get started by organizing your first memorable experience.</p>
            <Link href="/add-events">
                <button class="cursor-pointer flex items-center justify-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors shadow-md shadow-blue-600/20">
                    <Plus class="h-4 w-4" />
                    Create Event
                </button>
            </Link>
...
function submitEdit() {
    editForm.post(`/events/${editForm.id}`, {
        forceFormData: true,
