<template>
    ...
    <div class="p-6 space-y-6">
        ...
        <template v-else>
            ...
            <div v-if="!events.length" class="flex flex-col items-center justify-center py-20 px-4 text-center border rounded-lg bg-white shadow-sm">
                <div class="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4">
                    <LayoutTemplate class="h-8 w-8 text-slate-400" />
                </div>
                <h3 class="text-lg font-bold text-slate-900 mb-1">No events with seats</h3>
                <p class="text-sm text-slate-500 max-w-sm">You haven't added seat layouts to any of your events yet.</p>
            </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="event in paginatedItem" :key="event.eventID">
                            ...
                            <TableCell>
                                <Link :href="`/organizer/events/${event.eventID}/seats`" class="mt-5 block">
                                    <button class="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm">
                                        Manage Seats
...
import { LayoutTemplate, Loader2, ArrowUp, ArrowDown, ArrowUpDown, ArrowRight } from 'lucide-vue-next';
